...
import { Bell, Calendar, Check, Trash2, MailOpen, CheckCheck } from "lucide-react";
  ...
  return (
    <header className="flex justify-between items-center bg-white shadow px-6 py-4 relative z-50">
      ...
      <div className="flex gap-4 items-center">
        <Calendar className="cursor-pointer text-slate-600 hover:text-blue-600 transition-colors" />
              ...
              <div className="max-h-96 overflow-y-auto">
                {notifications.length === 0 ? (
                  ...
                  notifications.map((notif) => (
                    <div 
                      key={notif._id} 
                      className={`p-4 border-b border-slate-50 hover:bg-slate-50 flex justify-between gap-3 transition-colors ${!notif.isRead ? 'bg-blue-50/30' : ''}`}
                    >
